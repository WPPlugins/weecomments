...
class wee_mailerClass
{
	...
	function wee_sendScheduledMails()
	{
        ...
        //De momento el lang es el language default
        $lang = $this->weecomments_options['WEE_LANG'];
				...
				if ((int)$send_mail->send_mail) {
					...
					if (count($products)) {
						...
						foreach ($products as $product) {	
                            ...
                            $product_url = get_permalink($product->id_product);
                            $image_array = wp_get_attachment_image_src(get_post_thumbnail_id($product->id_product), 'full');
                            $product_photo = $image_array[0];
                            
                            $this->wee_getWebPage(
                                'http://weecomments.com/wsrest/add_product?api='.$this->weecomments_options['WEE_API_KEY'].'&lang='.$lang.'&id_product='.urlencode($product->id_product).'&product_name='.urlencode($product_name).'&product_url='.urlencode($product_url).'&product_photo='.urlencode($product_photo) );
                    ...
                    //Se envían los datos a weeComments para que envíe los emails
                    if ($this->weecomments_options['WEE_SEND_MAIL_FROM']) {
                        $text_results .= $this->wee_sendMailFromWee($order, $id_products_string);
                        sleep(rand(1, 2));
                    } else {
                        $text_results .= $this->wee_sendMail($order, $id_products_string);
                        sleep(rand(3, 10));
                    }
	...
	function wee_sendMail($order, $id_products_string)
	{
		$mail_html = $this->wee_prepareMailTemplate($order, $id_products_string);
    ...
    private function wee_sendMailFromWee($order, $id_products_string = "")
    {   
        $lang = $this->weecomments_options['WEE_LANG'];
        $order->id_store = 0;
        
        echo 'http://weecomments.com/wsrest/add_order?api='.$this->weecomments_options['WEE_API_KEY']
        .'&id_shop='.$this->weecomments_options['WEE_ID_SHOP']
        ...
        .'&date='.urlencode($order->date)
        .'&lang='.$lang;
        echo "<br>";
        
        $this->wee_getWebPage(
        'http://weecomments.com/wsrest/add_order?api='.$this->weecomments_options['WEE_API_KEY']
        .'&id_shop='.$this->weecomments_options['WEE_ID_SHOP']
        .'&id_store='.$order->id_store
        .'&id_order='.$order->id_order
        .'&customer_name='.urlencode($order->firstname)
	...
	function wee_prepareMailTemplate($order, $id_products_string)
	{
        //De momento el lang es el language default
        $lang = $this->weecomments_options['WEE_LANG'];
        
		$link = 'http://weecomments.com/'.$order->iso_code.'/new_review_form2?id_shop='
        ...
        $header_img = $this->weecomments_options['WEE_MAIL_HEADER'];
        
        if (strlen($header_img) > 0) {
            $htmlHeader = 
                '<tr>
                    <td align="center">
                        <img align="center" src="'.$header_img.'" style="max-width:100%; padding:0 !important; display:inline !important; vertical-align:top; border:0; min-height:auto; outline:none; text-decoration:none;"
                    </td>
                </tr>';
        } else {
            $htmlHeader = "";
		...
		$weemailClass->wee_cargarPlantilla(plugin_dir_path(__FILE__).'../mails/'.$lang.'/review_general.html');
        ...
        $weemailClass->wee_registrarCampo("WEE_COLOUR", $this->weecomments_options['WEE_COLOUR']);
        $weemailClass->wee_registrarCampo("WEE_COLOUR2", $this->weecomments_options['WEE_COLOUR2']);
        $weemailClass->wee_registrarCampo("HEADER", $htmlHeader);
        $weemailClass->wee_registrarCampo("WEE_MAIL_BUTTON", $this->weecomments_options['WEE_MAIL_BUTTON']);
	...
	function wee_updateShopInfo()
	{
		...
		$shop_info = $this->wee_getWebPage('http://weecomments.com/wsrest/module_shop_info?api='.$this->weecomments_options['WEE_API_KEY']);
		$shop_info = new SimpleXMLElement($shop_info);
        
        
        $this->wee_getWebPage('http://weecomments.com/wsrest/update_module_version?api='.$this->weecomments_options['WEE_API_KEY'].'&version=3.1.0');
	...
	{
		$langs_configuration = $this->wee_getWebPage(
	...
	{
		$response = $this->wee_getWebPage('http://weecomments.com/wsrest/shop_product_reviews2?api='.$this->weecomments_options['WEE_API_KEY']);
    ...
    function wee_sincronizeComments(){
		...
		$response = $this->wee_getWebPage('http://weecomments.com/wsrest/get_all_shop_comments?api='.$this->weecomments_options['WEE_API_KEY'].'&last_comment='.$last_comment);
    ...
    function wee_sincronizeCommentsReplies(){
		...
		$response = $this->wee_getWebPage('http://weecomments.com/wsrest/get_all_shop_comments_replies2?api='.$this->weecomments_options['WEE_API_KEY'].'&last_id_comment_reply='.$last_comment);
    ...
    private function wee_getWebPage($url)
    {
        $options = array(
